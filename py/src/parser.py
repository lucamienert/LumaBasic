...
class Parser:
    ...
    def expr(self):
        ...
        if self.current_tok.matches(Tokens.TT_KEYWORD, 'decl'):
            res.register_advancement()
            self.advance()
    ...
    def atom(self):
        ...
        elif tok.type == Tokens.TT_STRING:
            res.register_advancement()
            self.advance()
        ...
        elif tok.matches(Tokens.TT_KEYWORD, 'FUN'):
    ...
    def if_expr_b_or_c(self):
        ...
        if self.current_tok.matches(Tokens.TT_KEYWORD, 'elseif'):
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(Tokens.TT_KEYWORD, 'FUN'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'FUN'"
            ))
